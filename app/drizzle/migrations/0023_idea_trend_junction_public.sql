-- ============================================
-- Migration: idea_trend junction table (public schema)
-- ============================================
-- Creates junction table in public schema for Idea ↔ Trend N:M relationship

-- 1. Create junction table in public schema
...
-- 4. Also migrate from trend_id column if it exists and is not null
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'idea' AND column_name = 'trend_id'
  ) THEN
    INSERT INTO public.idea_trend (idea_id, trend_id, is_primary, created_at)
    SELECT
      i.id AS idea_id,
...
-- 5. Drop deprecated columns if they exist
